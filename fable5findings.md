...
- **c24aa4e1f protective-stop fills.** A resting broker stop that elected
  ...
  `protective_stop`, keyed on the client_order_id prefix.
- **f1784f92a option expiry / exercise / assignment.** These used to be
  recorded as `external_broker`. `AlpacaBroker.list_option_lifecycle_activities`
  reads GET /v2/account/activities (OPEXP/OPEXC/OPASN/OPTRD; parameters
  quoted from Alpaca's reference). order_sync now closes a vanished option as
  - `option_expired`, with an exact exit value of 0;
  - `option_exercised` or `option_assigned`, with the exit from the last mark.
  Exercise and assignment page the operator with the shares the paired OPTRD
  delivered: stock with no stop. With no event, or if the read fails, it
  falls back to the old external path.
- **0aef8aa6c EOD job.** Ghost P&L marking only ever ran inside
  `daily_cron.main`, which returns 2 before it when the LLM cannot be
  built. So **the Refusal Ledger has not been marked since the key was
  removed on 2026-09-11.**
  - `eod_report.run_eod` runs ghost marking, then a daily report, at
    21:15 UTC on trading days. That time is after the close in both EDT
    and EST.
  - The report goes to the log and to OPS_ALERT_WEBHOOK_URL in full. The
    push carries counts only, because push bodies are lock-screen-safe.
  - `EOD_REPORT_ENABLED` defaults on.
  - Webhook tasks are now referenced until they finish (RUF006, a real GC
    hazard).
- **ff1abeca5** A sub-cent GLM spend printed "$0.00". It now prints "<$0.01".
- **b53f5aee5 leader election.** A Postgres advisory lock is held on a
  dedicated AUTOCOMMIT connection. Only the holder runs the reconciler
  fleet and scheduler.
  - The heartbeat checks `pg_locks`, not just that the connection answers.
  - `/health` reports `"loops": leader|follower`.
  - `LEADER_ELECTION_ENABLED` defaults on.
- **6ac94d5bf unacknowledged orders.** A pending row with a NULL
  broker_order_id counted as an in-flight exit forever. That is how AAPL's
  stopped-out position read OPEN on 2026-09-04. Such a row now counts only
  within `UNACKED_ORDER_GRACE` (10 min). This replaces the "option_stops
  pending-row" item left open last session: `_place` persists only after
  the broker accepts, so the real gap was a half-written row, and it
  applies to every path.
- **1d9ce8cd9 flatten-all button** on the desktop Positions screen:
  - the confirm prompt names both effects, including auto-approve going off;
  - the result line lists whatever is still open;
  - it invalidates the broker-connections cache.
- **Handover fix (this docs commit).** Every build-log entry since
  2026-09-04 (0d2ad9486) had been inserted INSIDE the READ-THIS-FIRST
  banner, splitting its first sentence. As a result, CLAUDE.md's
  `sed -n '/^# Build log/,/^### /p'` showed the stale 2026-09-03 entry,
  not the newest. The entries now sit under the real heading, newest
  first. A line-multiset diff shows nothing was lost; only the split
  sentence was rejoined.
- **VERIFIED against a real Postgres 16.** It was a throwaway instance in
  the session scratchpad, not Railway:
  - `alembic upgrade head` applied cleanly through 0019_iv_history, the
    first time that migration met a real DB;
  - AdvisoryLease contention, handover and killed-backend detection;
  - two real processes booting the lifespan: A leads, B follows, A exits,
    B leads;
  - every `build_daily_report` query;
  - the lifecycle UPDATE (option_expired, -300.00);
  - the unacked-grace fix: fixed code closes a 1h-stale row's decision and
    keeps a 30s-old one in flight; old code leaves the stale one stuck open.
  - Full suite: 1860 passed, 12 skipped. Jest: 127. Every fix was
    revert-checked (see each commit).
- **NOT verified:** anything on Railway or Alpaca. That covers:
  - the activities endpoint, whose field names and signs come from Alpaca's
    documented examples;
  - whether COUNCIL_SCHEDULER_ENABLED=1 is set, which every scheduler loop
    (IV snapshot and EOD included) needs;
  - whether Railway's Postgres sits behind a transaction pooler, which
    would break session advisory locks, loudly;
  - the flatten-all button visually.
- **Deliberately NOT built, with reasons (recorded in PLAN_PLATFORM):**
  - Portfolio Greeks limits are deferred to Phase 5. The book is
    long-premium only, so worst loss is the premium, already capped
    (7.5% total, 1.5% per position, per-underlying, per-direction). Held
    positions also carry no underlying price, IV or delta in the risk
    context. Short legs in spreads are what make Greeks limits load-bearing.
  - The FOMC/CPI/NFP blackout is deferred as a veto. The sweep runs at
    10:00 ET, after the 8:30 CPI/NFP prints. These events land about every
    8 trading days, so a pre-event blackout would veto roughly half of all
    5-day holds on single names, where the event IV premium is small. It
    should come back as a backtested feature, not a rule. FRED release ids
    are confirmed for later: CPI 10, Employment Situation 50, FOMC Press
    Release 101. `include_release_dates_with_no_data=true` is what returns
    future dates.
- **Open:**
  - The delivered stock from an exercise is alerted, not auto-closed.
    Whether it should be is the operator's call.
  - The mobile (React Native) app has no flatten-all control.
  - Phases 5 and 5b still wait on a signal that clears the Phase 3 bar.
...
active worktrees, then merge what was ready. Two had genuinely finished,
unmerged work; two were already fully merged (one still locked by a
live `claude` process, left untouched).

**`claude/vigorous-burnell-2b057a`** (`430efe13`, own entry above) —
clean, isolated (`auth_store.py` + its own test), merged with only the
expected `fable5findings.md` append-conflict (resolved by
concatenation, per this repo's established pattern). Re-ran
`test_auth_hardening.py` (13/13) and the full workspace suite (1319
passed / 11 skipped) after.

**`claude/xenodochial-snyder-f1d0bd`** (`9b35b51d`, own entry above) —
genuinely conflicted with `2f974934` (this session's own desktop-login
fix): both rewrote the same `login.tsx`/`verify.tsx` catch blocks. That
branch reused the existing `runErrorMessage()`; `2f974934` introduced a
separate `authErrorMessage()` specifically because `runErrorMessage`'s
429 wording ("Daily council budget reached") is council-run-specific
and wrong on a login screen — a reason the other branch's own
reasoning never weighed (it only considered the "may still be starting
up" wording). Kept `authErrorMessage` for both auth screens; took the
other branch's `positions.tsx` / desktop `Positions.tsx` fixes as-is
(no overlap — those wire the open/closed-positions `ErrorState`s to
the real query error instead of a static string).

**Real bug this surfaced, fixed while reconciling:** that branch's own
commit message noted `runErrorMessage` already handles a FastAPI
validation-array `detail` (`{detail: [{loc, msg, type}]}`) by reading
`detail[0].msg`, calling it out as a fix over the old hand-rolled
extraction. `authErrorMessage` (written a few hours earlier, this
session) had NOT been given that same handling — its `String(detail)`
would render `"[object Object]"` on that exact shape. Extracted the
shared parsing into one `extractDetail()`, used by both
`runErrorMessage` and `authErrorMessage`, so this can't drift into a
third differently-buggy copy. Revert-checked per §4.1: reintroduced the
bare `String(detail)` version, confirmed the new
`authErrorMessage`/array-detail test fails with exactly
`"[object Object]"` (pasted below), restored, confirmed it passes.
...
**Also fixed, found while touching the same file:** `positions.tsx`'s
CLOSED-tab `ErrorState` had the identical static-string bug the merged
branch fixed for the open tab — `closedQuery.error` was never wired in
either, because the closed-positions tab was added to `main` after
that branch diverged and it never saw the pattern. One-line fix in the
same spot, same pattern, flagged here per §4.7 rather than silently
folded in as if it were part of either original branch's own work.

**Verified:** `tsc --noEmit` clean; full mobile suite 12 suites / 100
tests passed (99 + 1 new). Left alone: `agent-a00975d8662b9a692`
(fully merged already, still locked by a live process) and
`agent-a326fbc25255c4875` (fully merged, idle) — nothing pending in
either.
